...
namespace Destrospean.CmarNYCBorrowed
{
    public class GEOM : IComparable<GEOM>
    {
        ...
        public float GetTotalBoneWeight(int vertexIndex, List<uint> boneHashes)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (vertBones[i] < this.bonehasharray.Length && vertBones[i] >= 0 &&
                    boneHashes.Contains(this.bonehasharray[vertBones[i]])) weight += vertWeights[i] / 255f;
            }
            return weight;
        }
        ...
        public GEOM(GEOM sourceMesh)
        {
            ...
            this.numVerts = sourceMesh.VertexCount;
        ...
        public GEOM(GEOM basemesh, Vector3[] delta_positions, Vector3[] delta_normals)
        {
            ...
            if (basemesh.VertexCount != delta_positions.Length | basemesh.VertexCount != delta_normals.Length)
            {
            ...
            this.numVerts = basemesh.VertexCount;
            this.Fcount = 3;
        ...
        public void FillWaistGap()
        {
            ...
            for (int i = 0; i < this.seamStitches.Length; i++)
            {
                ...
                if (this.seamStitches[i].SeamType == maleWaist) maleVerts.Add(this.seamStitches[i]);
            }
        ...
        public void AppendMesh(GEOM meshToAppend)
        {
            ...
            Face[] newFaces = new Face[this.FaceCount + meshToAppend.FaceCount];
            Array.Copy(this.meshfaces, 0, newFaces, 0, this.FaceCount);
            for (int i = 0; i < meshToAppend.FaceCount; i++)
            {
                newFaces[i + this.FaceCount] = new Face(meshToAppend.meshfaces[i].facePoint0 + this.numVerts,
        ...
        public void AutoVertexID(GEOM refMesh)
        {
            if (!this.HasVertexIDs)
            {
                ...
                this.vertID = new int[this.VertexCount];
            }
            Vector3[] refVerts = new Vector3[refMesh.VertexCount];
        ...
        internal void BoneVectorize(RIG.Bone bone)
        {
            Vector3 bonePosition = bone.WorldPosition;
            this.deltaPosition = new Vector3[this.vPositions.Length];
            for (int i = 0; i < this.vPositions.Length; i++)
            {
                this.deltaPosition[i] = (new Vector3(this.vPositions[i].Coordinates)) - bonePosition;
            }
        }

        internal void BoneDeVectorize(RIG.Bone bone, float weight)
        {
            if (weight == 0f) return;
            Vector3 bonePosition = bone.WorldPosition;
            for (int i = 0; i < this.vPositions.Length; i++)
            {
                float vertWeight = GetBoneWeightForVertex(i, bone.BoneHash);
                if (vertWeight == 0f) continue;
                Vector3 oldPos = new Vector3(this.vPositions[i].Coordinates);
                Vector3 newPos = this.deltaPosition[i] + bonePosition;
                oldPos += (newPos - oldPos) * vertWeight * weight;
                this.vPositions[i] = new position(oldPos.Coordinates);
            }
        }

        internal void BoneMorpher2(RIG.Bone bone, float weight, Vector3 scale, Quaternion rotation)
        {
            if (weight == 0) return;
            ...
            for (int i = 0; i < this.vPositions.Length; i++)
            {
                ...
                this.deltaPosition[i] = this.deltaPosition[i].Scale(weightedScale);
                this.deltaPosition[i] = (weightedRotation * this.deltaPosition[i] * weightedRotation.Conjugate()).ToVector3();
            ...
            for (int i = 0; i < this.numVerts; i++)
            {
                ...
                for (int j = 0; j < head.numVerts; j++)
                {
                    if (iPos.positionClose(head.GetPosition(j)))
                    {
                        ...
                        uint tag = this.getTagval(i) & 0xFFFFFF00;
                        this.setTagval(i, tag | 0x00000063);
                    }
                }
            }
        }
        ...
        public void Clean()
        {
            ...
            List<uv>[] newUV = new List<uv>[this.UVCount];
        ...
        public class MTNF
        {
            ...
            public int normalIndex
            {
                get
                {
                    for (int i = 0; i < this.paramCount; i++)
                    {
                        if (this.paramList[i][0] == (uint)FieldType.NormalMap)
                        {
            ...
            public int emissionIndex
            {
                get
                {
                    for (int i = 0; i < this.paramCount; i++)
                    {
                        if (this.paramList[i][0] == (uint)FieldType.EmissionMap)
                        {
                            uint tmp = (uint)this.dataList[i][0];
                            return (int)tmp;
                        }
        ...
        {
            if (this.VertexCount > other.VertexCount) return -1;
            else if (this.VertexCount < other.VertexCount) return 1;
            else return 0;
        }
    }
}
